...
def show_parcours_exemple(df_source, nom_lycee):
    import pandas as pd
    representatifs = pd.read_csv("profils_representatifs.csv")
    st.header("📋 Exemple de parcours représentatif")
    df_repre = representatifs[representatifs["Lycée"] == nom_lycee]
    if df_repre.empty:
        st.info("Aucun parcours représentatif disponible pour ce lycée.")
    else:
        domaines = sorted(df_repre["Domaine"].dropna().unique())
        domaine_selected = st.selectbox("Sélectionner un domaine :", domaines, key=f"domaine_exemple_{nom_lycee}")
        exemples = df_repre[df_repre["Domaine"] == domaine_selected]
        if not exemples.empty:
            if st.button("🔁 Un autre exemple", key=f"autre_exemple_{nom_lycee}"):
                st.session_state[f"exemple_index_{nom_lycee}"] = st.session_state.get(f"exemple_index_{nom_lycee}", 0) + 1
            exemple_index = st.session_state.get(f"exemple_index_{nom_lycee}", 0) % len(exemples)
            profil_url = exemples.iloc[exemple_index]["Profile URL"]
            profil_match = df_source[df_source["Profile Public URL"] == profil_url]
            if not profil_match.empty:
                profil_row = profil_match.iloc[0].to_dict()
                st.subheader(f"🧭 Domaine : {domaine_selected}")
                st.markdown(f"**💼 Poste :** {profil_row.get('Job Title', 'Non spécifié')} chez {profil_row.get('Company Name', '')}")
                st.markdown(f"**📚 Dernier diplôme :** {profil_row.get('Diplôme Titre #1', '')} ({profil_row.get('Niveau Diplôme #1', '')} - {profil_row.get('Domaine Diplôme #1', '')})")
                st.markdown(f"**🏫 Établissement :** {profil_row.get('Établissement Diplôme #1', '')}")
                st.markdown(f"**🏷️ Statut :** {profil_row.get('Catégorie socio-professionnelle', '')}")
                st.markdown(f"**🎓 Domaine :** {profil_row.get('Domaine du dernier poste', '')}")
                st.markdown("### 🧾 Parcours détaillé")
                for ligne in afficher_parcours_complet(profil_row):
                    st.markdown(f"- {ligne}")
            else:
                st.warning("Profil non retrouvé dans les données sources.")

# ----------- 🧭 Fonctions d'affichage ----------- #
def show_tabs(df, nom_lycee):

    sous_tabs = st.tabs(["🎓 Diplômes", "💼 Postes", "📋 Exemple de parcours"])
...
#                     st.success("Profil enregistré avec succès.")
    ...
    with sous_tabs[2]:
        show_parcours_exemple(df, nom_lycee)
